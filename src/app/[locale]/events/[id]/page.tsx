...
import { Calendar, Users, Clock, MapPin, Star, Wine, ChefHat, Music, ArrowLeft, Phone, Mail } from 'lucide-react';
...
const eventsData = [
  {
    ...
    description: 'Experience a curated selection of premium Italian wines paired with authentic antipasti and live acoustic music in our cozy restaurant atmosphere.',
    description_de: 'Erleben Sie eine kuratierte Auswahl an Premium-italienischen Weinen, gepaart mit authentischen Antipasti und Live-Akustikmusik in unserer gemütlichen Restaurant-Atmosphäre.',
...
Diese intime Erfahrung ist auf 30 Gäste begrenzt, um persönliche Aufmerksamkeit und die perfekte Atmosphäre für Weingenuss und Gespräche zu gewährleisten.`,
    ...
    featured: true,
    image: '🍷',
    highlights: ['6 premium Italian wines', 'Artisanal antipasti pairings', 'Live acoustic music', 'Expert sommelier guidance', 'Wine region education', 'Take-home tasting notes'],
    includes: [
      'Welcome aperitif',
  ...
  },
  {
    ...
    description: 'Learn the secrets of perfect pasta preparation from our head chef in this hands-on cooking experience where tradition meets technique.',
    description_de: 'Lernen Sie die Geheimnisse der perfekten Pasta-Zubereitung von unserem Chefkoch in diesem praktischen Kocherlebnis, wo Tradition auf Technik trifft.',
...
Der Kurs endet mit einem gemeinsamen Abendessen, bei dem Sie die von Ihnen zubereiteten Pasta-Gerichte genießen, kombiniert mit von unserem Sommelier ausgewählten Weinen. Sie erhalten auch gedruckte Rezepte und Tipps, um diese Gerichte zu Hause nachzukochen.`,
    ...
    featured: true,
    image: '🍝',
    highlights: ['3 pasta varieties', 'Professional techniques', 'Take-home recipes', 'Chef guidance', 'Communal dinner', 'Wine pairings'],
    includes: [
  ...
  },
  {
    ...
    description: 'Indulge in our exclusive truffle menu featuring fresh Italian truffles in a luxurious 5-course tasting experience.',
    description_de: 'Verwöhnen Sie sich mit unserem exklusiven Trüffel-Menü mit frischen italienischen Trüffeln in einem luxuriösen 5-Gänge-Degustationserlebnis.',
...
Der Abend beinhaltet Wein-Paarungen, die speziell ausgewählt wurden, um die erdigen, komplexen Aromen von Trüffeln zu ergänzen, zusammen mit einer Bildungskomponente über Trüffel-Jagd-Traditionen und kulinarische Anwendungen.`,
    ...
    featured: true,
    image: '🍄',
    highlights: ['Fresh Italian truffles', '5-course tasting', 'Premium wine pairings', 'Limited availability', 'Educational experience', 'Luxury dining'],
    includes: [
...
export default async function EventPage({ params }: { params: Promise<{ id: string; locale: string }> }) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      event = {
        ...
        description: dbEvent.description || 'Enjoy this special event at our restaurant.',
        description_de: dbEvent.description || 'Genießen Sie diese besondere Veranstaltung in unserem Restaurant.',
        longDescription: dbEvent.description || 'Join us for this special event at La Cantina Berlin. More details will be provided upon booking.',
        longDescription_de: dbEvent.description || 'Begleiten Sie uns zu dieser besonderen Veranstaltung im La Cantina Berlin. Weitere Details erhalten Sie bei der Buchung.',
        date: dbEvent.date,
        time: '19:00',
        duration: '3 hours',
        ...
        featured: true,
        image: '🍽️',
        highlights: ['Authentic Italian cuisine', 'Professional service', 'Cozy atmosphere', 'Fresh ingredients', 'Wine pairings', 'Memorable experience'],
        includes: [
          'Welcome drink',
          'Multi-course meal',
  ...
  const description = locale === 'de' ? event.description_de || event.description : event.description;
  const longDescription = locale === 'de' ? event.longDescription_de || event.longDescription : event.longDescription;
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-12">
              ...
              {/* What's Included */}
              <div className="bg-card/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg border border-border">
                <h2 className="text-3xl font-serif font-bold text-foreground mb-6">{t('whats_included')}</h2>
              ...
              {/* Schedule */}
              <div className="bg-card/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg border border-border">
                <h2 className="text-3xl font-serif font-bold text-foreground mb-6">{t('event_schedule')}</h2>
                <div className="space-y-4">
                  {event.schedule.map((item, index) => (
                    <div key={index} className="flex items-start gap-4 pb-4 border-b border-border/30 last:border-b-0">
              ...
              {/* Highlights */}
              <div className="bg-card/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg border border-border">
                <h2 className="text-3xl font-serif font-bold text-foreground mb-6">{t('event_highlights')}</h2>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-card/80 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border border-border sticky top-6">
                <div className="text-center mb-6">
                  <div className="text-4xl font-bold text-primary mb-2">€{event.price}</div>
                  <div className="text-muted-foreground">{t('per_person')}</div>
                </div>

                <div className="space-y-4 mb-6">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">{t('available_spots')}:</span>
                    <span className="font-semibold text-foreground">{event.availableSpots} / {event.capacity}</span>
                  </div>
                ...
                <div className="pt-6 border-t border-border/30">
                  <h4 className="font-semibold text-foreground mb-4">{t('need_help')}</h4>
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <Phone className="w-4 h-4" />
                      <a href="tel:+4930883215600" className="hover:text-primary transition-colors">030 8832156</a>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <Mail className="w-4 h-4" />
                      <a href="mailto:info@ristorante-la-cantina.de" className="hover:text-primary transition-colors">
              ...
              <div className="bg-card/50 backdrop-blur-sm rounded-2xl p-6 border border-border">
                <h3 className="font-semibold text-foreground mb-4">{t('event_details')}</h3>
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('dress_code')}:</span>
                    <span className="font-medium">{event.dresscode}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('age_restriction')}:</span>
                    <span className="font-medium">{event.ageRestriction}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('languages')}:</span>
                    <span className="font-medium">{event.languages.join(', ')}</span>
                  </div>
                  <div className="pt-2 border-t border-border/30">
                    <span className="text-muted-foreground text-xs">{event.dietary}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Location */}
              <div className="bg-card/50 backdrop-blur-sm rounded-2xl p-6 border border-border">
                <h3 className="font-semibold text-foreground mb-4">{t('location')}</h3>
                <div className="space-y-2 text-sm">
                  <div className="flex items-start gap-2">
                    <MapPin className="w-4 h-4 text-primary mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="font-medium">La Cantina Berlin</p>
