...
export default function ReservationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage('🎉 Ihre Reservierung wurde automatisch bestätigt! Wir freuen uns auf Ihren Besuch bei La Cantina Berlin.');
        setFormData({
          name: '',
          phone: '',
          email: '',
        ...
        setMessage('Fehler beim Senden der Reservierung. Bitte versuchen Sie es erneut.');
      }
    } catch (error) {
      setMessage('Fehler beim Senden der Reservierung. Bitte versuchen Sie es erneut.');
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      >
        {isSubmitting ? 'Wird gesendet...' : t('submit')}
      </button>
    </form>
  );
}