...
export default function MenuWithFilters({ menuItems, locale }: MenuWithFiltersProps) {
  ...
  // Main sections
  const mainSections = ['All', 'Food', 'Drinks'];
  ...
  const isDrink = (item: MenuItem): boolean => {
    const category = (locale === 'de' ? item.category_de || item.category : item.category_en || item.category).toLowerCase();
  ...
  const isAlcoholic = (item: MenuItem): boolean => {
    const category = (locale === 'de' ? item.category_de || item.category : item.category_en || item.category).toLowerCase();
    const title = (locale === 'de' ? item.title_de || item.title : item.title_en || item.title).toLowerCase();
    const description = (locale === 'de' ? item.description_de || item.description : item.description_en || item.description).toLowerCase();
  ...
  const renderMenuItem = (item: MenuItem) => {
    const title = locale === 'de' ? item.title_de || item.title : item.title_en || item.title;
    const description = locale === 'de' ? item.description_de || item.description : item.description_en || item.description;
